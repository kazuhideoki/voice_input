...
impl MemoryMonitor {
    ...
    pub fn add_usage(&self, additional_bytes: usize) {
        let current = self.current_usage.fetch_add(additional_bytes, Ordering::SeqCst);
        ...
        #[cfg(debug_assertions)]
        println!("[DEBUG] Memory usage increased by {} bytes, total: {} MB", additional_bytes, mb);
    ...
    pub fn subtract_usage(&self, freed_bytes: usize) {
        ...
        let mb = new_usage / 1024 / 1024;
        #[cfg(debug_assertions)]
        println!("[DEBUG] Memory usage decreased by {} bytes, total: {} MB", freed_bytes, mb);
    }
