...
pub async fn start_recording() -> Result<(Option<String>, ()), Box<dyn Error>> {
    // Get selected text at recording start
...
pub async fn stop_recording_and_transcribe(start_selected_text: Option<String>) -> Result<String, Box<dyn Error>> {
    ...
    match start_selected_text {
        Some(text) if !text.trim().is_empty() => {
            println!("Using text selected at recording start as context: {:?}", text);
