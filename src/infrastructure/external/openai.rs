...
impl OpenAiClient {
    ...
    /// ファイルパスとプロンプトで転写
    pub async fn transcribe_with_prompt(&self, audio_file_path: &str, prompt: Option<&str>) -> Result<String, String> {
        // ファイル読み込み
    ...
    /// 共通の転写処理
    async fn transcribe_with_part(&self, file_part: multipart::Part, prompt: Option<&str>) -> Result<String, String> {
...
pub async fn transcribe_audio(
    ...
    client.transcribe_with_prompt(audio_file_path, prompt)
        .await
        .map_err(|e| Box::new(std::io::Error::new(std::io::ErrorKind::Other, e)) as Box<dyn std::error::Error>)
}
