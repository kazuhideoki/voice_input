...
fn compare_recording_modes() {
    ...
    println!(
        "{:<10} {:<20} {:<20} {:<20}",
        "Duration", "Memory Time", "File Time", "Speedup"
    );
    println!("{:-<70}", "");
...
fn benchmark_memory_monitor_overhead() {
    ...
    let mut recorder_with_monitor =
        Recorder::new(backend_with_monitor).with_memory_monitor(monitor.clone());
    let start = Instant::now();
