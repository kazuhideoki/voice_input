...
fn test_stack_mode_prevents_auto_paste() {
    ...
    let should_paste_normal = paste && (stack_service_none.is_none() || !stack_service_none.as_ref().unwrap().borrow().is_stack_mode_enabled());
    ...
    let should_paste_stack = paste && (stack_service_some.is_none() || !stack_service_some.as_ref().unwrap().borrow().is_stack_mode_enabled());
...
fn test_stack_mode_paste_logic_comprehensive() {
    ...
    let should_paste = paste && (stack_service_ref.is_none() || !stack_service_ref.as_ref().unwrap().borrow().is_stack_mode_enabled());
    assert!(!should_paste, "paste=false should never paste regardless of stack mode");
    ...
    let should_paste = paste && (stack_service_ref.is_none() || !stack_service_ref.as_ref().unwrap().borrow().is_stack_mode_enabled());
    assert!(!should_paste, "paste=true with stack mode ON should NOT paste");
    
    // ケース4: paste=true, stack_service=None（レガシーモード）
    let stack_service_ref: Option<Rc<RefCell<StackService>>> = None;
    let should_paste = paste && (stack_service_ref.is_none() || !stack_service_ref.as_ref().unwrap().borrow().is_stack_mode_enabled());
    assert!(should_paste, "paste=true with no stack service should paste (legacy behavior)");
}
