...
- `text_input.worker` が短文で大きい（200〜500ms）
...
**狙い**: `text_input.worker` の起動コスト削減  
**理由**: 短文での占有率が高い（200〜500ms）

やること
- `enigo_helper` を常駐プロセスにし、stdin/IPC でテキストを送信
- 失敗時は現行方式にフォールバック（理由: 安定性維持）
- 計測: `text_input.worker` がどれだけ縮むか確認

期待効果
- 200〜500ms 程度の短縮（短文ほど効果が大きい）

対応状況（2026-01-31 時点）
- 常駐ワーカー化: 対応済み（同一プロセス内の別スレッド）
- subprocess での常駐化: 未対応（不要になったため削除済み）
...
対応状況（2026-01-31 時点）
- `std::thread::sleep` の置換: 対応済み（current_thread 影響なし。残存はワーカー/テストのみ）
- `osascript` の非同期化: 未対応
