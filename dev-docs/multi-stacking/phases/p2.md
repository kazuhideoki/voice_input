...
#### 完了条件
- [ ] 全てのCLIコマンドが正常に実行できる
- [ ] スタックモードのON/OFF切り替えが動作する
- [ ] スタックの保存/取得/一覧表示/クリアが動作する
- [ ] 番号指定によるペースト機能が動作する
- [ ] 全てのユニットテストが成功する
...
- [ ] **1-6: スタック保存・取得メソッドの実装**
  - save_stack()
  - get_stack()

- [ ] **1-7: スタック一覧・クリアテストの作成**
  ```rust
  #[test]
...
- [ ] **2-4: CLIコマンドハンドリングの実装**
  - match文での新規コマンド処理
  - 適切なIpcCmd変換

#### Task 3: デーモン側IPC処理更新

- [ ] **3-1: StackService統合テストの作成**
  ```rust
...
1. **コマンドパースチェック**
   - [ ] `voice_input stack-mode on` がエラーなく解析される
   - [ ] `voice_input paste 1` が適切な番号で解析される
   - [ ] `voice_input --help` に新しいコマンドが表示される
