...
実APIで観測した注意点は `survey/FINDINGS.md` に記録しています。

## Setup

```sh
python3 -m venv .venv
.venv/bin/python -m pip install -r survey/requirements.txt
```
...
```sh
.venv/bin/python survey/realtime_whisper_benchmark.py --audio sample.wav
```

出力は `survey/runs/YYYYMMDDTHHMMSS/` に保存されます。
...
`manifest.json` と `results.*` には音源末尾の診断も入ります。`likely_truncated_audio=true` の場合、音声が十分な無音で終わっておらず、Realtime 文字起こしでは最後の単語が欠ける可能性があります。
...
```sh
# 複数回実行
.venv/bin/python survey/realtime_whisper_benchmark.py --audio sample.wav --repetitions 3

# デフォルトはファイル検証向けに最速送信。
.venv/bin/python survey/realtime_whisper_benchmark.py --audio sample.wav --send-interval-scale 0

# 実時間ストリームに近い速度で送る。
.venv/bin/python survey/realtime_whisper_benchmark.py --audio sample.wav --send-interval-scale 1
...
.venv/bin/python survey/realtime_whisper_benchmark.py --audio sample.wav --realtime-vad

# 片方だけ実行
.venv/bin/python survey/realtime_whisper_benchmark.py --providers gpt4o_stream
.venv/bin/python survey/realtime_whisper_benchmark.py --providers realtime_whisper
```
